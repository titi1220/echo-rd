...
const steps = ["Persona", "Última vez", "Foto", "Contacto", "Revisión"];
    ...
    <form action={formAction} className="grid gap-6">
      ...
      <section className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          ...
          <Field label="Rasgos físicos"><textarea name="physical_traits" className={textareaClass} required /></Field>
        </div>
      </section>

      <section className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
        <h2 className="text-xl font-black text-civic">Paso 2: Datos de la última vez visto</h2>
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          ...
          <Field label="Lugar exacto donde fue visto por última vez"><input name="location_last_seen" className={inputClass} required /></Field>
          <Field label="Fecha en que fue visto por última vez"><input name="date_last_seen" type="date" className={inputClass} required /></Field>
          <Field label="Hora en que fue visto por última vez"><input name="time_last_seen" type="time" className={inputClass} required /></Field>
      ...
      <section className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
        ...
        <p className="mt-3 text-sm leading-6 text-slate-600">Límite recomendado: 5 MB. JPG, PNG o WebP.</p>
      </section>

      <section className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
        <h2 className="text-xl font-black text-civic">Paso 4: Información de contacto</h2>
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          <Field label="Nombre de contacto"><input name="contact_name" className={inputClass} required /></Field>
          <Field label="Teléfono"><input name="phone" className={inputClass} required /></Field>
          <Field label="WhatsApp"><input name="whatsapp" className={inputClass} required /></Field>
          <Field label="Correo electrónico"><input name="email" type="email" className={inputClass} required /></Field>
      ...
      <section className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
        ...
        <label className="mt-4 flex gap-3 text-sm font-semibold leading-6 text-slate-700">
          ...
          Confirmo que tengo autorización para enviar este caso y acepto que Echo RD lo revise antes de publicarlo.
        </label>
        <CaptchaNotice />
        {state.message && (
        ...
        <button disabled={pending} className="mt-5 w-full rounded-xl bg-alert px-6 py-4 font-black text-white disabled:opacity-60">
          {pending ? "Enviando..." : "Enviar caso para revisión"}
        </button>
      </section>
    </form>
  );
