...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const description = `${item.full_name}, ${item.age} anos. Visto por ultima vez en ${item.province}, ${item.municipality}.`;
  return {
    title,
    description,
    openGraph: {
    ...
    <main className="bg-white">
      <section className="mx-auto grid max-w-7xl gap-8 px-4 py-8 sm:px-6 md:grid-cols-[.9fr_1.1fr] lg:px-8">
        <div className="relative overflow-hidden rounded-3xl bg-slate-100 shadow-civic">
          <div className="relative aspect-[4/5]">
            <Image src={item.photo_url} alt={`Foto de ${item.full_name}`} fill className="object-cover" priority />
          </div>
        ...
        <div className="flex flex-col justify-center">
          <div className="mb-4 flex flex-wrap gap-2">
            <Badge tone={item.urgency_level === "critical" ? "red" : "amber"}>{item.urgency_level === "critical" ? "Urgencia critica" : "Prioridad alta"}</Badge>
            {item.verified && <Badge tone="blue">Verificado por moderacion</Badge>}
          </div>
          <h1 className="text-4xl font-black tracking-tight text-civic sm:text-5xl">{item.full_name}</h1>
          <p className="mt-2 text-lg font-bold text-slate-500">{item.age} anos {item.nickname ? `| "${item.nickname}"` : ""}</p>
      ...
      <section className="border-y border-slate-200 bg-mist py-10">
        <div className="mx-auto grid max-w-7xl gap-5 px-4 sm:px-6 md:grid-cols-3 lg:px-8">
          {[
            ...
            ["Rasgos fisicos", item.physical_traits],
      ...
      <section className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="rounded-3xl border border-blue-100 bg-blue-50 p-6">
          ...
          <h2 className="mt-3 text-xl font-black text-civic">Proteccion de identidad</h2>
          <p className="mt-2 leading-7 text-slate-700">
            Las pistas, reportes y datos de quienes escriben son privados. Solo admins autorizados pueden revisarlos para apoyar la verificacion.
          </p>
        </div>
      </section>
    </main>
